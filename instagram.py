...
Cada card es 1080x1350 px (formato retrato 4:5) con el diseno del blog.
"""
...
def _build_card_html(sport_name, sport_icon, picks, stats=None, site_url="parleysports90.github.io/sports-blog"):
    """Genera el HTML de un card para una liga/deporte."""
    today = datetime.now().strftime("%d %b %Y").upper()

    picks_html = ""
    for pick in picks[:6]:  # max 6 picks por card
        conf = pick.get("confidence", 0)
        conf_class = "high" if conf >= 75 else ("medium" if conf >= 60 else "low")
        conf_pct = min(conf, 100)

        home = pick.get("home_abbr") or pick.get("home_team", "")
        away = pick.get("away_abbr") or pick.get("away_team", "")
        ...
        home_logo_tag = f'<img src="{home_logo}" class="team-logo" onerror="this.style.display='none'">' if home_logo else ""
        away_logo_tag = f'<img src="{away_logo}" class="team-logo" onerror="this.style.display='none'">' if away_logo else ""

        league_badge = f'<span class="league-badge">{league}</span>' if league and league != sport_name else ""
...
def generate_instagram_images(predictions, tracking_data=None, output_dir=OUTPUT_DIR):
    """
    Genera un card PNG por deporte que tenga picks.
    Retorna lista de rutas de archivos generados.
    """
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("  [Instagram] playwright no instalado. Ejecuta: pip install playwright && playwright install chromium")
        return []
    ...
    with sync_playwright() as pw:
        ...
        for sport_key, sport_data in predictions.items():
            ...
            # Si hay multiples ligas (ej futbol), agrupar por liga
            by_league = {}
            for pick in picks:
                league = pick.get("league", sport_name)
                by_league.setdefault(league, []).append(pick)

            # Un card por liga si hay mas de una; un card por deporte si es 1 liga
            if len(by_league) == 1:
                cards = [(sport_name, sport_icon, picks)]
            else:
                cards = [(league, sport_icon, league_picks)
            ...
            for card_name, icon, card_picks in cards:
                ...
                print(f"  [Instagram] Card generado: {out_path}")
